...
public partial class MechMovementMotor : MovementMotor
{
    ...
    public virtual void FixedUpdate()//transform.position += targetVelocity * Time.deltaTime * walkingSpeed * 3;
    {
        ...
        {
            targetVelocity = (this.transform.forward * this.walkingSpeed) + (this.GetComponent<Rigidbody>().linearVelocity.y * Vector3.up);
        }
        else
        {
            targetVelocity = this.GetComponent<Rigidbody>().linearVelocity.y * Vector3.up;
        }
        this.GetComponent<Rigidbody>().linearVelocity = Vector3.MoveTowards(this.GetComponent<Rigidbody>().linearVelocity, targetVelocity, (Time.deltaTime * this.walkingSpeed) * 3);
    }
