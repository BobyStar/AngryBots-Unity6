...
public partial class SpiderAnimationTest : MonoBehaviour
{
    ...
    {
        float w = 0.0f;
        this.rigid.linearVelocity = ((Quaternion.Euler(0, this.angle, 0) * this.rigid.transform.forward) * 2.4f) * this.walking;
        Vector3 velocity = this.rigid.linearVelocity;
        velocity.y = 0;
        ...
        float angle = Mathf.DeltaAngle(SpiderAnimationTest.HorizontalAngle(this.tr.forward), SpiderAnimationTest.HorizontalAngle(this.rigid.linearVelocity));
        if (angle < -90)
        {
